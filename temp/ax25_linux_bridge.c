...
typedef struct {
    ...
    void (*on_ui)(uint8_t conn_id, const uint8_t *data, uint16_t len, uint8_t pid, void *ctx);
    void *app_ctx;
...
int ax25_bridge_send_ui(const char *dest_call, const uint8_t *data, uint16_t len, uint8_t pid) {
    ...
    // Build the frame header: C/R=1 marks this as a command UI frame.
    memset(&hdr, 0, sizeof(hdr));
    hdr.destination = *dest;
    hdr.source = *src;
    hdr.cr = 1U;

    // Populate the UI frame struct identically to T34 encode path.
...
int ax25_bridge_open_kernel_monitor(const char *ifname) {
        ...
        if (fd >= 0) {
            ...
            if (ioctl(fd, SIOCGIFINDEX, &ifr) == 0) {
                ...
                if (bind(fd, (struct sockaddr*) &sll, sizeof(sll)) == 0) {
                    ...
                    hal_log(HAL_LOG_WARN, "AF_PACKET bind(%s): %s", ifname, strerror(errno));
                    close(fd);
                    fd = -1;
                }
            } else {
                hal_log(HAL_LOG_WARN, "SIOCGIFINDEX %s: %s", ifname, strerror(errno));
                close(fd);
                fd = -1;
            }
        } else {
            hal_log(HAL_LOG_WARN, "AF_PACKET unavailable (need CAP_NET_RAW): %s — "
                    "falling back to AF_AX25/SOCK_DGRAM (UI frames only)", strerror(errno));
        }
    }
    ...
    if (fd < 0) {
        ...
        if (bind(fd, (struct sockaddr*) &sa, sizeof(sa)) < 0) {
            hal_log(HAL_LOG_WARN, "AF_AX25 bind(%s): %s", ifname ? ifname : "", strerror(errno));
            close(fd);
            return -1;
        }
        if (ifname && *ifname) {
            ...
            memset(&ifr, 0, sizeof(ifr));
            strncpy(ifr.ifr_name, ifname, IFNAMSIZ - 1U);
            setsockopt(fd, SOL_SOCKET, SO_BINDTODEVICE, ifr.ifr_name, (socklen_t) strlen(ifr.ifr_name));
        }
...
void ax25_bridge_poll_kernel(void) {
    ...
    if (g_ctx.kernel_sock_is_packet) {
        ...
        if (n <= 0)
            return;
        hal_log(HAL_LOG_INFO, "Kernel RX (raw): %u bytes on ifindex=%d", (uint16_t) n, sll.sll_ifindex);
    } else {
        ...
        hal_log(HAL_LOG_INFO, "Kernel RX (UI): %u bytes from %s", (uint16_t) n, src_call);
    }
...
void ax25_bridge_deinit(void) {
    ...
    for (i = 0U; i < AX25_BRIDGE_MAX_CONNS; i++) {
        if (g_ctx.conns[i].active) {
            ...
            g_ctx.conns[i].active = 0U;
            // start modified part
            // B9 FIX: remove in-loop decrement; num_conns zeroed unconditionally below
            // end modified part
        }
    }
...
void ax25_bridge_set_ui_handler(uint8_t conn_id, void (*fn)(uint8_t, const uint8_t*, uint16_t, uint8_t, void*), void *ctx) {
    if (conn_id >= AX25_BRIDGE_MAX_CONNS)
        return;
